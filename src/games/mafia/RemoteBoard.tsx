...
import { type MafiaRemoteState, type MafiaRemoteAction } from "./remote";

interface Props extends GameComponentProps {
  remote: RemoteContext;
}

export const MafiaRemoteBoard: React.FC<Props> = ({ players, remote, onComplete, onQuit }) => {
  const startedAt = useMemo(() => Date.now(), []);
  const state = remote.state as MafiaRemoteState | null | undefined;
  const me = remote.myPeerId;
  const isHost = remote.isHost;
  const dispatch = remote.dispatch as (a: MafiaRemoteAction) => void;
  ...
  useEffect(() => {
    if (!state) return;
    if (state.kind === "night") playCue(MAFIA_CUES.nightIntro);
    else if (state.kind === "day-resolve") playCue(state.killedId ? MAFIA_CUES.dayKilled : MAFIA_CUES.daySafe);
    else if (state.kind === "day-vote") playCue(MAFIA_CUES.dayVote);
    else if (state.kind === "day-voted-out")
      playCue(state.eliminatedId ? MAFIA_CUES.dayVotedOut : MAFIA_CUES.dayTie);
    else if (state.kind === "end")
      playCue(state.winningTeam === "town" ? MAFIA_CUES.townWins : MAFIA_CUES.mafiaWins);
  }, [state]);
  ...
  useEffect(() => {
    ...
    const winnerIds = state.players
      .filter((p) => (state.winningTeam === "town" ? p.role !== "mafia" : p.role === "mafia"))
      .map((p) => p.peerId);
    onComplete({
      ...
      highlights: [`${state.winningTeam === "town" ? "Town" : "Mafia"} won`],
    });
  ...
  if (state.kind === "end") {
    const accent = state.winningTeam === "town" ? "text-[hsl(var(--ember))]" : "text-[#b94a4a]";
    return (
      <RoomCodeBar code={remote.code}>
        <section className="mx-auto max-w-md animate-fade-up">
          ...
          <h2 className={`mt-2 font-display text-5xl italic ${accent}`}>
            {state.winningTeam === "town" ? "Town wins." : "Mafia wins."}
          </h2>
  ...
  if (state.kind === "night") {
    ...
    const renderRolePrompt = () => {
      ...
      if (myPlayer.role === "mafia") {
        const myVote = state.mafiaVotes[me];
        const fellowMob = alive.filter((p) => p.role === "mafia");
        ...
        return (
          <div className="mt-4 rounded-lg border border-[hsl(0_70%_55%/0.5)] bg-[hsl(0_70%_55%/0.08)] p-4">
            <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-[hsl(0_70%_55%)]">Mafia</p>
            <p className="mt-1 text-xs text-muted">Fellow mafiosi: {fellowMob.map((w) => w.name).join(", ")}</p>
            <p className="mt-3 text-sm">Pick who to hit tonight. Majority wins; ties broken at random.</p>
            <ul className="mt-3 space-y-1.5">
              {targets.map((t) => {
                const votesForT = Object.entries(state.mafiaVotes).filter(([, v]) => v === t.peerId).map(([w]) => w);
                return (
                  <li key={t.peerId}>
                    <button
                      type="button"
                      onClick={() => dispatch({ type: "mafia-vote", targetId: t.peerId })}
            ...
            <button
              type="button"
              onClick={() => dispatch({ type: "mafia-vote", targetId: null })}
      ...
      if (myPlayer.role === "detective") {
        const mySub = state.detectiveSubmissions[me];
        ...
        return (
          <div className="mt-4 rounded-lg border border-[hsl(210_80%_65%/0.5)] bg-[hsl(210_80%_65%/0.08)] p-4">
            <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-[hsl(210_80%_65%)]">Detective</p>
            <p className="mt-1 text-sm">Pick someone to investigate. You&apos;ll learn their alignment.</p>
            <ul className="mt-3 space-y-1.5">
              {targets.map((t) => (
                <li key={t.peerId}>
                  <button
                    ...
                    onClick={() => dispatch({ type: "detective-check", targetId: t.peerId })}
                    className={`block w-full rounded-md border px-3 py-2 text-left text-sm transition-colors ${
                      mySub === t.peerId
            ...
            {mySub === undefined && (
              <button
                type="button"
                onClick={() => dispatch({ type: "detective-check", targetId: null })}
                className="mt-3 w-full rounded-md border border-border py-2 font-mono text-[11px] uppercase tracking-wider text-muted hover:text-fg"
              >
                Skip
              </button>
      ...
      if (myPlayer.role === "doctor") {
        ...
        return (
          <div className="mt-4 rounded-lg border border-[hsl(140_60%_55%/0.5)] bg-[hsl(140_60%_55%/0.08)] p-4">
            ...
            <p className="mt-1 text-sm">Pick someone to save tonight.</p>
      ...
      return (
        <div className="mt-4 rounded-lg border border-border bg-bg/40 p-4 text-center text-sm text-muted">
          The town sleeps. Close your eyes.
        </div>
      );
    };
  ...
  if (state.kind === "day-resolve") {
    ...
    const myRead = state.detectiveReads.find((r) => r.detectiveId === me);
    return (
      <RoomCodeBar code={remote.code}>
        <section className="mx-auto max-w-md animate-fade-up text-center">
          <p className="font-mono text-[11px] uppercase tracking-[0.3em] text-muted">Morning · night {state.round}</p>
          {killed ? (
            <>
              <h2 className="mt-2 font-display text-4xl italic text-[#b94a4a]">{killed} is gone.</h2>
              <p className="mt-3 text-sm text-muted">The mafia struck.</p>
            </>
          ) : (
            <>
              <h2 className="mt-2 font-display text-4xl italic text-[hsl(var(--ember))]">Everyone&apos;s here.</h2>
              <p className="mt-3 text-sm text-muted">Either the mafia abstained or the doctor intervened.</p>
            </>
          )}
          {myRead && (
            <div
              ...
              key={myRead.targetId}
            >
              <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-[hsl(210_80%_65%)]">Detective — private</p>
              <p className="mt-1 font-display text-xl italic">
                {findName(myRead.targetId)} is on the <span className="font-bold">{myRead.team}</span> side.
              </p>
            </div>
          )}
          {isHost && (
    ...
    return (
      <RoomCodeBar code={remote.code}>
        <section className="mx-auto max-w-md animate-fade-up text-center">
          ...
          {name ? (
            <>
              <h2 className="mt-2 font-display text-4xl italic">{name} is run out.</h2>
